...
test('exportFileName stamps the local date and time into the name', () => {
    assert.equal(exportFileName(new Date(2026, 6, 30, 14, 38, 0)), 'retirement-plan-20260730T143800.json');
});

test('exportFileName pads every field to two digits, so the names stay the same length and sort correctly', () => {
    assert.equal(exportFileName(new Date(2026, 0, 5, 9, 7, 3)), 'retirement-plan-20260105T090703.json');
});
