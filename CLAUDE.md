...
* `Economy.js` extends `Base` -- shared market/inflation assumptions read once from `cfg.Economy`: `inflationRate`, `interestRate`, `sp500Rate`. `Bookkeeper` builds one and exposes it as `bookkeeper.economy`, so any class with a `bookkeeper` reference uses the shared rate instead of an independently-configured one of its own. `colaRate` is also exposed, but not read from cfg -- it's derived internally as `this.colaRate = this.inflationRate`, hidden from clients (`SocialSecurity.js` still reads `bookkeeper.economy.colaRate`, unaware it's just `inflationRate`); a future scenario that needs COLA and general inflation to diverge would give `colaRate` its own cfg input here instead. `Mortgage.rate` is deliberately not here -- it's a fixed loan rate, not a market/inflation assumption. `sp500Rate` is a getter, not a plain property, so every existing caller (`Account.growthRate()`, `HsaAccount`'s drawdown calc) keeps reading `bookkeeper.economy.sp500Rate` unchanged while the value underneath can now vary by year: `Bookkeeper.runYear()` sets `economy.currentYear` every year, and the getter returns `crashSequence.get(currentYear)` when a `RobustnessValidator` trial has called `setCrashSequence()` (a `Map<year, rate>` from `MarketCrash.js`), else falls back to the configured `_baseSp500Rate` unchanged -- `crashSequence` is `null` for every non-`--robustness` run, so this is a zero-behavior-change addition for the optimizer/`--debug`/every existing test. `baseSp500Rate` (a separate getter) always returns the configured rate regardless of any active crash, for the rare one-time planning calculation (`HsaAccount`'s amortized drawdown) that must not lock in a single crash year's anomalous rate for its whole multi-decade schedule.
* `RandomTable.js` -- a fixed, committed array of ~2000 pre-generated `[0,1)` floats (literal data, not a re-seedable PRNG call -- "seed table" per the former TODO). `randomValue(startYear, year, trial)` indexes `TABLE[(year - startYear + trial * TRIAL_STRIDE) % TABLE.length]` (`TRIAL_STRIDE = 47`, prime relative to the table length, so trials read well-separated slices instead of nearly-overlapping windows) -- every call for the same `(year, trial)` returns the same value forever, so a `RobustnessValidator` run building many `Bookkeeper`/`Simulator` instances per process (one per trial) gets trials that are comparable to each other and reproducible across a later rerun.
* `MarketCrash.js` -- `buildCrashSequence({startYear, endYear, annualProbability, crashes, trial})` computes one trial's full year -> magnitude `Map` once, up front (not re-rolled lazily year by year, so every account touching `sp500Rate` in a trial sees the identical crash years). Each year, if `randomValue(...) < annualProbability`, that year crashes: `sp500Rate` is replaced by `crashes[i].rate` for that year only, where `i` cycles through `cfg.MarketCrash.crashes` **in the array's own configured order** (not drawn randomly among them, so a stress run is traceable to a specific real historical sequence and reproducible from `cfg.json` alone -- reordering the array changes which magnitude a run applies first). "Non-recurring" is a one-year cooldown: the year immediately after a crash always has zero crash probability, then normal odds resume.
...
* `RobustnessValidator.js` -- deliberately a separate step run *after* the optimizer, not part of it: `run(configData, classes, trials = 200)` takes `configData` exactly as given, same "no candidate substitution" contract as `--debug` (whatever `categoryOrder`/ceilings/`claimAge` are already sitting in `cfg.json` -- presumably the optimizer's own winning values, copied in by hand, see the `Cash.categoryOrder`/etc `cfg.json` note), and stress-tests *that one plan* against `trials` different `MarketCrash.js` sequences (`cfg.MarketCrash`) instead of searching for a better one -- it answers "how fragile is the plan we already picked," not "what's the best plan." Builds one `Bookkeeper`/`Simulator` per trial, calls `bookkeeper.economy.setCrashSequence(buildCrashSequence({..., trial}))` before running it, and catches `InsufficientFundsError` per trial the same way `Optimizer.runAll()` catches it per-candidate, so one bad trial doesn't abort the batch. `report()` prints the insolvency rate (count and %, plus the failure-year range) and a net-worth distribution (min/p10/median/p90/max) instead of a single Net Worth number -- an average alone would hide exactly what this exists to surface, how often the plan fails outright and how wide the spread is.
...
`RandomTable.js`/`MarketCrash.js`/`Economy.js`'s `sp500Rate` getter (see
their own Modularization bullets above) implement the three original TODO
items directly: a literal committed seed table (not a re-seedable PRNG),
a non-recurring (one-year-cooldown) crash drawn from a probability and
applied to that year's `sp500Rate` only, and Monte Carlo trials via
`RobustnessValidator`. `cfg.MarketCrash` (`annualProbability`, `crashes`
-- an ordered, cycling list of historical magnitude/name pairs) is fully
configurable, matching the "set cfg.json up" request once it became clear
that request was about the withdrawal-ceiling values, not this -- the
crash model just happened to also end up configurable, for the same
reason every other assumption in this project is (`Economy`, tax
brackets, Medicare premiums): so real numbers can replace illustrative
placeholders without touching code.

Wiring a real crash (a negative `sp500Rate`, possible for the first time
in this project's history) through the real config surfaced a genuine
pre-existing bug, not a new one: `TaxableAccount._checkBasis()`'s
