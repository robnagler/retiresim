...
* `Economy.js` extends `Base` -- shared market/inflation assumptions read once from `cfg.Economy`: `inflationRate`, `interestRate`, `sp500Rate`. `Bookkeeper` builds one and exposes it as `bookkeeper.economy`, so any class with a `bookkeeper` reference uses the shared rate instead of an independently-configured one of its own. `colaRate` is also exposed, but not read from cfg -- it's derived internally as `this.colaRate = this.inflationRate`, hidden from clients (`SocialSecurity.js` still reads `bookkeeper.economy.colaRate`, unaware it's just `inflationRate`); a future scenario that needs COLA and general inflation to diverge would give `colaRate` its own cfg input here instead. `Mortgage.rate` is deliberately not here -- it's a fixed loan rate, not a market/inflation assumption. `sp500Rate` is a getter, not a plain property, so every existing caller (`Account.growthRate()`, `HsaAccount`'s drawdown calc) keeps reading `bookkeeper.economy.sp500Rate` unchanged while the value underneath can now vary by year: `Bookkeeper.runYear()` sets `economy.currentYear` every year, and the getter returns `historicalReturns.get(currentYear)` when a `RobustnessValidator` trial has called `setHistoricalReturns()` (a `Map<year, rate>` from `HistoricalReturns.js`'s `buildReturnSequence()`), else falls back to the configured `_baseSp500Rate` unchanged -- `historicalReturns` is `null` for every non-`--robustness` run, so this is a zero-behavior-change addition for the optimizer/`--debug`/every existing test. `baseSp500Rate` (a separate getter) always returns the configured rate regardless of any active historical-return sequence, for the rare one-time planning calculation (`HsaAccount`'s amortized drawdown) that must not lock in a single sampled year's anomalous return for its whole multi-decade schedule.
...
* `HistoricalReturns.js` -- `ANNUAL_RETURNS` is a committed array of real S&P 500 nominal annual total returns, 1965-2025 (61 values, sourced from published historical return tables), living in JavaScript, **not** `cfg.json` -- market history is a fact, not a per-user assumption, so it's not configurable (same spirit as `Medicare.js`'s `IRMAA_BRACKETS`). Uses each *full calendar year's* real return rather than a hand-picked "crash magnitude": nobody actually liquidates at a crash's exact bottom -- money stays invested and spending only draws down what a given month needs, so a few bad months blend into the rest of the year. 2020 is the clearest example: the COVID crash happened inside it, but the full year still returned +18.40%. `buildReturnSequence({startYear, endYear, trial})` computes one trial's full year -> return `Map` once, up front (not re-rolled lazily, so every account touching `sp500Rate` in a trial sees identical draws) -- **every** year, unconditionally, picks one of the 61 real annual returns uniformly at random via `randomValue(startYear, year, trial)`, with replacement. Deliberately **not** a chronological replay of real history (there's no reason simulated 2026 should behave like real 1965 just because both are "first"), and deliberately has **no** separate "does this year crash" gate, magnitude table, or cooldown -- superseded design (see the Robustness validator narrative below): the real annual figures already contain both ordinary and crash years in their true historical proportions and relative frequency, so uniform sampling reproduces that mix without a hand-tuned probability or magnitude list, and back-to-back bad years (e.g. 1973 then 1974) can be drawn together, same as real history -- running many trials is what surfaces how often that particular bad luck actually matters.
...
`RandomTable.js`/`HistoricalReturns.js`/`Economy.js`'s `sp500Rate` getter
(see their own Modularization bullets above) implement the three original
TODO items, though the crash-specific two of the three ended up
...
Back-to-back bad years (e.g. 1973 then 1974) are a real, un-suppressed
possible outcome now, not prevented by a cooldown -- running many trials
is what surfaces how often that specific bad luck matters. This data is a
committed JavaScript constant, not `cfg.json` -- market history is a fact,
not a per-user assumption, unlike everything else this project exposes via
config (`Economy`, tax brackets, Medicare premiums) specifically so real
numbers can replace illustrative placeholders without touching code; this
